...
def get_stat_by_quiz(username: str, quiz_id: int):
    ...
    query = """
        SELECT tr.score, tr.status, tr.quiz_id, q.title
    ...
    return {"found": False, "score": 0, "status": "Немає даних", "quiz_id": quiz_id, "title": "Невідомий тест"}
...
def save_result(data: TestResult):
    try:
        ...
        conn.commit()
        return {"message": "Результат збережено і юзера додано"}
        
    except Exception as e:
        return {"message": f"Помилка: {str(e)}"}
    finally:
